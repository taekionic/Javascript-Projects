...
function Input_Digit (digit) {
    ...
    //we are checking to see if Wait_second_operand is true and set Display Value to the key that was clicked. 
...
function Handle_Operator(Next_Operator) {
    ...
    //when an operator key is pressed, we convert the current number displayed on screen to a number and store the result in Calculator.First_Operand if it doesn't already exist.
...
keys.addEventListener('click', (event) => {
    //the target variable is an object that represents the object that was clicked
    const { target } = event;
